...
class ResNet18_cifar10(object):
    ...
    def getdataloader(self, datatype, train_db_path, test_db_path, batchsize):
        # get transformations
        transform_train, transform_test = self._gettransforms(datatype=datatype)
        n_classes = 0
    ...
    def train(self, epoch, criterion, optimizer, logfile, loader, device):
        print(f"\nEpoch: {epoch}")
        self.net.train()
        iteration = 0
        train_loss = 0
    ...
    def savemodel(self, acc, epoch, device, save_dir):
        state = {
            'net': self.net.modules if device == 'mps' else self.net,
            'acc': acc,
            'epoch': epoch,
        }
        if not os.path.isdir(save_dir):
            os.mkdir(save_dir)
        torch.save(state, os.path.join(save_dir, 'resmet18_cifar10_' + str(epoch) + '.pth'))